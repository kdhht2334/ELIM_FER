...
class MlpMixer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=1000, patch_size=16, 
        ...
        self.btl_head = nn.Linear(config.hidden_dim, latent_dim, bias=False)
        self.bn = nn.BatchNorm1d(latent_dim, affine=True)
    ...
    def forward(self, x, labels=None):
        ...
        logits = self.head(x)
        latent_variable = self.btl_head(x)  #F.relu(self.bn(self.btl_head(x)))
...
if __name__ == "__main__":
    ...
    num_classes = 2
    model = MlpMixer(config, img_size=224, num_classes=num_classes, patch_size=16, zero_head=False)
    model.load_from(np.load('/media/CVIP1/FER/dh/mlpmixer_checkpoint/INet_1K/Mixer-B_16.npz'))
    
    from pytorch_model_summary import summary as pt_summary
    print(pt_summary(model, torch.ones_like(torch.empty(1, 3, 224, 224)), show_input=True))
